...
export default function StudentRegistrationForm() {
  ...
  const handleSearchChange = (field, value) => {
    if (field === "sekolah") {
      ...
      const existingOption = sekolahOptions.find((option) => option.label.toLowerCase() === value.toLowerCase());
      ...
      // Add option if it doesn't exist
      const existingOption = jurusanOptions.find((option) => option.label.toLowerCase() === value.toLowerCase());
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await axios.post(`${import.meta.env.VITE_API_URL}/peserta`, formDataToSend, {
        headers: {
  ...
  return (
    <div className="max-w-6xl mx-auto bg-white p-5 rounded-lg">
      ...
      <div className="border-b border-gray-200 py-4 px-2 mb-4">
        ...
        <p className="text-sm text-gray-500">Silahkan Lengkapi Data diri anda</p>
      </div>
      ...
      <div className="flex flex-col lg:flex-row">
        ...
        <div className="w-full lg:w-1/2 space-y-4 px-2">
          ...
          <div className="mb-4">
            ...
            <input type="text" name="nama" placeholder="Nama" className="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-1 focus:ring-blue-500" value={formData.nama} onChange={handleChange} required />
          </div>
          ...
          <div className="grid grid-cols-2 gap-4 mb-4">
            ...
            <div>
              ...
              <div className="relative">
                <input type="date" name="tanggal_lahir" className="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-1 focus:ring-blue-500" value={formData.tanggal_lahir} onChange={handleChange} required />
                <div className="absolute inset-y-0 right-0 flex items-center px-2 pointer-events-none">
                  <svg className="w-4 h-4 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          ...
          <div className="grid grid-cols-12 gap-4 mb-4">
            ...
            <div className="col-span-8">
              <label className="block text-sm font-medium text-gray-700 mb-1">Sekolah/Universitas</label>
              <div className="relative" ref={sekolahDropdownRef}>
                ...
                {sekolahDropdownOpen && (
                  <div className="absolute z-10 mt-1 w-full bg-white border border-gray-300 rounded-md shadow-lg max-h-60 overflow-auto">
                    {sekolahOptions
                      .filter((option) => option.label.toLowerCase().includes(sekolahSearch.toLowerCase()))
                      .map((option, index) => (
                        <div key={index} className="px-3 py-2 cursor-pointer hover:bg-blue-50" onClick={() => handleSelectOption("sekolah", option.value, option.label)}>
                          {option.label}
                        </div>
                      ))}
                    {sekolahSearch && !sekolahOptions.some((option) => option.label.toLowerCase() === sekolahSearch.toLowerCase()) && (
                      <div
            ...
            <div className="col-span-4">
              <label className="block text-sm font-medium text-gray-700 mb-1">NISN/NIM</label>
              <input type="text" name="nisn" placeholder="NISN/NIM" className="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-1 focus:ring-blue-500" value={formData.nisn} onChange={handleChange} required />
            </div>
          </div>
          ...
          <div className="grid grid-cols-2 gap-4 mb-4">
            ...
            <div>
              ...
              <div className="relative" ref={jurusanDropdownRef}>
                ...
                {jurusanDropdownOpen && (
                  <div className="absolute z-10 mt-1 w-full bg-white border border-gray-300 rounded-md shadow-lg max-h-60 overflow-auto">
                    {jurusanOptions
                      .filter((option) => option.label.toLowerCase().includes(jurusanSearch.toLowerCase()))
                      .map((option, index) => (
                        <div key={index} className="px-3 py-2 cursor-pointer hover:bg-blue-50" onClick={() => handleSelectOption("jurusan", option.value, option.label)}>
                          {option.label}
                        </div>
                      ))}
                    {jurusanSearch && !jurusanOptions.some((option) => option.label.toLowerCase() === jurusanSearch.toLowerCase()) && (
                      <div
                        className="px-3 py-2 cursor-pointer bg-blue-50 text-blue-700 font-medium"
                        onClick={() => {
                          // Add to options
            ...
            <div>
              ...
              <div className="relative">
                <select name="kelas" className="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-1 focus:ring-blue-500 appearance-none bg-white" value={formData.kelas} onChange={handleChange} required>
        ...
        <div className="w-full lg:w-1/2 px-2 relative">
          ...
          <div className="mb-6">
            <label className="block text-sm font-semibold text-gray-700 mb-2">Masukkan Foto Diri Disini</label>
            ...
            <div className="flex flex-row items-center space-x-4">
              ...
              <div className="w-20 h-20 flex-shrink-0 rounded-full bg-gray-100 border border-gray-300 overflow-hidden flex items-center justify-center">
                {previewUrl ? <img src={previewUrl} alt="Preview" className="w-full h-full object-cover" /> : <img src={defaultAvatar} alt="Default Avatar" className="w-12 h-12 object-cover" />}
              </div>
              ...
              <div className="flex-grow bg-blue-50 border border-dashed border-blue-300 rounded-lg flex flex-col items-center justify-center text-center p-6 relative">
                ...
                <p className="text-xs text-gray-500 mt-1">SVG, PNG, JPEG OR GIF (max 1080x1200px)</p>
                <input type="file" name="foto" className="absolute inset-0 w-full h-full opacity-0 cursor-pointer" onChange={handlePhotoUpload} accept="image/*" />
              </div>
            </div>
          </div>
          ...
          <div className="mb-6">
            <label className="block text-sm font-semibold text-gray-700 mb-2">Masukkan CV Disini</label>
          ...
          <div className="absolute bottom-0 right-0 px-2 pb-6 pt-4">
            <button onClick={handleSubmit} className="py-2 px-6 bg-blue-500 text-white rounded-md hover:bg-blue-600 focus:outline-none focus:ring-2 focus:ring-blue-500">
              Kirim
            </button>
          </div>
        </div>
      ...
      {showSuccessModal && <SuccessModal isOpen={showSuccessModal} onClose={handleModalClose} />}
    </div>
  );
}
